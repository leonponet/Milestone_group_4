...
df <- df %>%
  mutate(Sub_Substance_Name_EN = case_when(
...
substance_summary <- df %>%
  count(Sub_Substance_Name_EN, sort = TRUE)
...
province_substance_summary <- df %>%
  group_by(Province, Sub_Substance_Name_EN) %>%
  summarise(count = n(), .groups = "drop")

# Create the stacked bar chart
ggplot(province_substance_summary, aes(x = Province, y = count, fill = Sub_Substance_Name_EN)) +
